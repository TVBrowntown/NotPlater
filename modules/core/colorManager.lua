...
-- modules/core/colorManager.lua
-- Simplified and fixed nameplate color management system with proper null checking
...
local function SafeGetNameplateInfo(frame)
    ...
    return playerName, level, nameText
end

-- Helper function to check if frame is a valid nameplate
local function IsValidNameplate(frame)
    if not frame then return false end
    
    -- Check if it has the basic nameplate structure
    if not frame.GetRegions then return false end
    
    local playerName = SafeGetNameplateInfo(frame)
...
function ColorManager:GetClassColorFromAllSources(frame, playerName, unit)
    ...
    -- 5. Direct unit detection from all possible units
    local _, level = SafeGetNameplateInfo(frame)
    local units = self:FindAllUnitsForName(playerName, level)
...
-- Update nameplate appearance (main entry point) - FIXED with proper null checking
function ColorManager:UpdateNameplateAppearance(frame)
    -- Safety checks
    if not frame then return end
    if not IsValidNameplate(frame) then return end
...
-- Simplified class check (for compatibility) - FIXED with proper null checking
function ColorManager:ClassCheck(frame)
    -- Safety checks
    if not frame then return false end
    if not IsValidNameplate(frame) then return false end
